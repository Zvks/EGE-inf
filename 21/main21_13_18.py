...
def game(col_rocks, start_heap_1, pl_position, v_hod1, v_hod2, v_hod3, winner): #р - позиция игрока. если четная, то значит, ход Вани, если нет - ход Пети 
    if pl_position == 4 and col_rocks + start_heap_1 >= winner:
        s = 1
    elif pl_position == 4 and col_rocks + start_heap_1 < winner:
        s = 0
    elif col_rocks + start_heap_1 >= winner and pl_position < 4:
        s = 0 
    else:
